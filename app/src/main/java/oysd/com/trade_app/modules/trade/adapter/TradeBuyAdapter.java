...
public class TradeBuyAdapter extends BaseAdapter<SocketOrder.BuyEntrustListBean> {


    public TradeBuyAdapter(@NonNull Context context, @NonNull int layoutId) {
        super(context, layoutId);
    }

    public TradeBuyAdapter(@NonNull Context context, int layoutId,
                            @Nullable List<SocketOrder.BuyEntrustListBean> dataSet) {
        super(context, layoutId, dataSet);
    }

    @Override
    protected void onBind(BaseHolder holder, SocketOrder.BuyEntrustListBean bb, int position) {
        ...
        holder.setText(R.id.tv_list_index,(position+1)+"");
    }
