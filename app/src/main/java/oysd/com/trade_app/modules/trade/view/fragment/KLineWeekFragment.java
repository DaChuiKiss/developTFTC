 ...
 */
public class KLineWeekFragment extends LazyLoadBaseFragment implements KChartView.KChartRefreshListener {
    ...
    public void getCacheValue() {
        ...
        String response = mCache.getAsString("kline1week" + ((KLineTradeActivity) getActivity()).getMarketId());
        if (response == null) {
    ...
    public void kilineSocketWeek() {
        disposable = SocketClient.getInstance().topicKLineInfo(((KLineTradeActivity) getActivity()).getMarketId(), 24 * 60 * 7 * 60 * 1000, new SocketClient.OnSocketReceivedListener() {
            @Override
            public void onReceive(String response) {
                ...
                try {
                    ...
                    if (obj.has("data")) {
                        isObject = false;
                        mCache.put("kline1week" + ((KLineTradeActivity) getActivity()).getMarketId(), response);
                        SocketKLine sk = gson.fromJson(response, SocketKLine.class);
                        List<SocketKLine.DataBean> lsdb = sk.getData();
                        for (int i = 0; i < lsdb.size(); i++) {
                            KLineEntity kLineEntity = new KLineEntity();
