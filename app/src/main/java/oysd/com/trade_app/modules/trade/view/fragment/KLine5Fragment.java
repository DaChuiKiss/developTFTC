 ...
 */
public class KLine5Fragment extends LazyLoadBaseFragment implements KChartView.KChartRefreshListener {
 ...
 */
   ...
   public void onFragmentResume() {
    ...
    public void getCacheValue(){
        ...
        String response =  mCache.getAsString("kline5min"+((KLineTradeActivity)getActivity()).getMarketId());
        if(response==null) {
    ...
    boolean isObject;
    public void kilineSocket5(){
        disposable = SocketClient.getInstance().topicKLineInfo(((KLineTradeActivity)getActivity()).getMarketId(),5*60*1000, new SocketClient.OnSocketReceivedListener(){
            @Override
            public void onReceive(String response) {
                ...
                try {
                    ...
                    if(obj.has("data")) {
                        isObject=false;
                        mCache.put("kline5min"+((KLineTradeActivity)getActivity()).getMarketId(),response);
                        SocketKLine sk = gson.fromJson(response, SocketKLine.class);
                        List<SocketKLine.DataBean> lsdb = sk.getData();
                        for (int i = 0; i < lsdb.size(); i++) {
                            KLineEntity kLineEntity = new KLineEntity();
