 ...
 */
public class KLine15Fragment extends LazyLoadBaseFragment implements KChartView.KChartRefreshListener {
    ...
    public void kilineSocket15() {
        disposable = SocketClient.getInstance().topicKLineInfo(((KLineTradeActivity) getActivity()).getMarketId(), 15 * 60 * 1000, new SocketClient.OnSocketReceivedListener() {
            @Override
            public void onReceive(String response) {
                ...
                try {
                    ...
                    if (obj.has("data")) {
                        isObject = false;
                        mCache.put("kline15min" + ((KLineTradeActivity) getActivity()).getMarketId(), response);
    ...
    public void getCacheValue() {
        ...
        String response = mCache.getAsString("kline15min" + ((KLineTradeActivity) getActivity()).getMarketId());
        if (response == null) {
