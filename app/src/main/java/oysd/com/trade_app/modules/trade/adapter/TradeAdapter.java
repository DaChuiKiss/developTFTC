 ...
 */
public class TradeAdapter extends BaseAdapter<MarketListBean> {

    public TradeAdapter(@NonNull Context context, @NonNull int layoutId) {
        super(context, layoutId);
    }

    public TradeAdapter(@NonNull Context context, int layoutId,
    ...
    protected void onBind(BaseHolder holder, MarketListBean marketListBean, int position) {
        ...
        holder.setText(R.id.coinUSDTprice, EmptyUtils.getBigDecimalValue(marketListBean.getNewestPrice()));

        holder.setText(R.id.coinRMBprice, marketListBean.getSymbol() + EmptyUtils.getBigDecimalValue(marketListBean.getLegalTenderVal()));

        holder.setText(R.id.volum_24h, "24H " + String.valueOf(Utils.getKtotal(marketListBean.getVolume())) + marketListBean.getSellCoinName());
